...
_inroot = '/work/ROMO/2016platform/CMAQv531/2016fh/12US2/POST'
inpaths = [
    '/home/bhenders/TestData/MCIP/GRIDCRO2D.12US2.35L.160101',
    f'{_inroot}/O3/O3_8HRMAX.LST.Y_24.2016fh.v531.12US2.5-9.ncf',
    f'{_inroot}/dailyavg/dailyavg.LST.Y_24.2016fh.v531.12US2.01.ncf',
...
for inpath, varkey in zip(inpaths, varkeys):
    filename = os.path.basename(inpath).replace('12US2', '108US2')
    outpath = (
        'input/'
        + filename.replace('.ncf', '').replace('.nc', '')
        + '.nc'
    )
    ...
    if 'dailyavg' in inpath:
        ...
        tmpf = tmpf.slice(TSTEP=tidx)
    outf = tmpf.apply(
        ROW=lambda y: y[:-3].reshape(-1, 9).mean(1),
        COL=lambda x: x.reshape(-1, 9).mean(1)
    )
